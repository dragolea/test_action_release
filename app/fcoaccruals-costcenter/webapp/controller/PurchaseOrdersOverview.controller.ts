...
export default class PurchaseOrdersOverview extends BaseController {
  ...
  /**
   * Updates the processing state of orders and their items from CostCenterResponsible to Controlling,
    ...
    orders.forEach((order) => {
      if (order.to_OrderItems !== null) {
        order.to_OrderItems.forEach((orderItem) => {
          if (orderItem.ProcessingState_code === constants.ProcessingState.CCR && orderItem.ApprovedByCCR === true) {
            promises.push(
              this.updateProperty(
                `/OrderItems(PurchaseOrder='${orderItem.PurchaseOrder}',PurchaseOrderItem='${orderItem.PurchaseOrderItem}')`,
                'ProcessingState_code',
                constants.ProcessingState.CONTROLLING,
              ),
            );
          }
        });
  ...
  /**
   * Handles the confirmation checkbox event, updating the 'ApprovedByCCR' property
  ...
  async onChangeHistoryPress(event: Event) {
    ...
    const promise = Fragment.load({
      id: view.getId(),
      name: 'de.freudenberg.fco.accruals.costcenter.view.fragment.ChangeHistory',
      controller: this,
